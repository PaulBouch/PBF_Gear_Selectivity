...
Survey_laa5 <- rbind(all_laa_NS, all_laa_igfs)  
save(Survey_laa5, file = "Survey_laa5.RData")

ggplot(Survey_laa5, aes(x= as.numeric(Age), y = model_laa, colour = Survey))+
  facet_wrap(~Species)+
  geom_line()
...
### raw survey data
save(igfs_5, NS_IBTS_5, file = "ALK/raw_survey_5.RData")
